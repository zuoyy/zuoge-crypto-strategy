...
## 模式 5：min_reward_risk 边界冲突（TP1=1.5R 被量化吃掉）

**症状**：信号全部被后端 reject，reason = `min_reward_risk`，但 TP ladder 正确构建（TP1=1.5R, TP2=2.2R）。

**根因**：TP1 精确等于 1.5R 时，量化取整可能让实际 R:R 跌到 1.5 以下（例如 1.48），而后端校验用 `>=` 或 `>` 的边界判断会拒绝。

**示例**：
```
price=$0.50, stop=$0.49（2% stop, 实际距离=$0.01）
TP1 目标 = $0.50 + 1.5×$0.01 = $0.515
量化取整（tick=0.001）→ $0.514（向下取整）
实际 R:R = ($0.514-$0.50) / ($0.50-$0.49) = $0.014/$0.01 = 1.40
min_reward_risk=1.5 → 1.40 < 1.5 → REJECTED
```

**修复**：将策略 `_apply_execution_constraints()` 的 `min_reward_risk` 从 `"1.5"` 降到 `"1.2"`。TP ladder 不变（仍 TP1=1.5R, TP2=2.2R），只放宽后端校验。量化取整导致的 0.1-0.3R 偏差被 1.2 兜住。

**注意**：不要改 SDK 的 `tp1_ratio`（影响所有策略）。策略文件的 `min_reward_risk` 是本策略专属的后端校验参数。实际 TP 目标位置不受影响。
